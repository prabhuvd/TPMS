...
class Radio():
    ...
    __vehicle_rf_config =[                
                    ...
                    "21 20 3C 00 01 00 00 00 07 00 00 00 00 00 07 00 00 00 50 00 00 01 B6 4C 66 00 00 05 02 03 01 01 01 01", #RC_CMD_RADIO_RX_DPROC_2 
    ...
    def write_command(self,UART_CmdTx):
        # This flag will decided to Turn ON the receiver again.
        #print "send:",UART_CmdTx
        if(0== self.__portInstance.write(self.hexToByte(UART_CmdTx))):
    ...
    def read_cmd_response(self):
        read_1_byte = self.byteToHex(self.__portInstance.read(1))
        ...
        if(len(read_byte_array)>=1):
            ...
            rx_byte= self.byteToHex(self.__portInstance.read(rsp_length))         
    ...
    def get_temperature_F(self,msg):
        return int(msg[6],16) 
    
    def get_pressure_PSI(self,msg):
        return int(msg[5],16)*0.2  
    
    def get_TPM_ID(self,msg):
        tpm_id = ""
        ...
        if(first_byte > 6):
            ...
            if(len(sensor_msg)>6):
                ...
                if(self.get_TPM_ID(sensor_msg) in self.__sensor_ids):
                    self.__sensor_ids[extracted_id] = [self.get_pressure_PSI(sensor_msg),self.get_temperature_F(sensor_msg)]
